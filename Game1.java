...
public class Game1 extends JPanel implements ActionListener,KeyListener {
	...
	public void draw(Graphics g) {
			...
			g.fillRect(0 , 0 , constants.WIDTH , constants.HEIGHT );
				...
				g.drawString("Score:"+foodEaten, (constants.WIDTH-font.stringWidth("Score:"+foodEaten))/4 , g.getFont().getSize() );
				if(Highscore.equals("")) {
					Highscore=this.GetHighScore1();
				}
				g.setFont(new Font("TimesRoman",Font.BOLD,20));
				FontMetrics font2=getFontMetrics(g.getFont());
				g.drawString("Highscore:"+Highscore, (constants.WIDTH-font2.stringWidth("Highscorecore:"+Highscore)) , g.getFont().getSize() );
		}
		else {
			gameOver(g);
	...
	public void CheckScore1() {
		...
		if( foodEaten>Integer.parseInt((Highscore.split(":")[1])) ) {
			String name="Normal-Ehtimam";
			Highscore=name+":"+foodEaten;
			File scoreFile=new File("Highscore1.txt");
			if(!scoreFile.exists()) {
...
public String GetHighScore1() {
	...
	public void start() {
		newFood();
		running = true;
		timer = new Timer(constants.FAST,this);
		timer.start();
	}
 ...
 public void checkFood() {
		...
		public void gameOver(Graphics g) {
			...
			g.drawString("Score:"+foodEaten, (constants.WIDTH-font1.stringWidth("Score:"+foodEaten))/4 , g.getFont().getSize() );
			g.setColor(Color.BLUE);
